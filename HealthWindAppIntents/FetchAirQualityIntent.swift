...
struct FetchAirQualityIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        while locationVM.latitude == 0.0 && locationVM.longitude == 0.0 {
            try await Task.sleep(nanoseconds: 1_000_000_000_000) // Espera 0.5 segundos
        }
        ...
        if let index = airQualityData.indexes.first {
            let responseMessage = """
            La calidad de aire actual en tu ubicación es (\(index.category)).\ 
